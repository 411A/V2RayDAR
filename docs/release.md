...
Active probing requires `sing-box`, which is downloaded separately. On first run in TUI mode (`--tui`), V2RayDAR asks for the local `sing-box` executable path, verifies it with `sing-box version`, saves it in `data.db`, and then starts scanning. In the default quiet mode there is no interactive prompt: V2RayDAR prints OS-specific setup instructions and exits until a bundled or configured `sing-box` executable is available.
