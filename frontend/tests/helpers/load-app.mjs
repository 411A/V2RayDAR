...
export function makeSandbox(overrides = {}) {
  ...
  for (const id of [
    ...
    "cfg-body", "cfg-empty",     "btn-sub-add", "btn-sub-import", "btn-sub-export", "sub-note", "sub-body", "sub-empty",
    "settings-note", "settings-groups", "btn-settings-reload", "btn-settings-reset",
    "dlg-reset", "dlg-reset-cancel", "dlg-reset-ok",
    "proxy-off", "proxy-local", "proxy-lan",
    "btn-proxy-unpin", "proxy-pill", "proxy-kv", "proxy-manual", "log-filter",
    "log-level", "log-follow", "btn-log-clear", "log-list", "log-empty", "btn-sharing",
    "share-list", "share-hint", "qr-img", "qr-note", "btn-qr-generate",
    "dlg-sub", "dlg-sub-form", "dlg-sub-title", "dlg-sub-url", "dlg-sub-name",
    "dlg-sub-priority", "dlg-sub-enabled", "dlg-sub-ok",
    "dlg-sub-import", "dlg-sub-import-form", "dlg-sub-import-ok", "dlg-sub-import-paste", "sub-import-text",
    "dlg-sub-export", "dlg-sub-export-copy", "sub-export-text", "dlg-detail",
...
const EXPORT_HOOK = `;globalThis.__v2 = {
  ...
  renderStats,   fmtClock, fmtStamp, fmtDuration, fmtAgo, fmtBytes, fmtLatency, fmtLatencyMs, LOG_LEVELS, LOG_LEVEL_ORDER, logLineVisible, parseLogLine, ltr, flagFor, extractFlag, flagCode, countryFlag, displayName,
