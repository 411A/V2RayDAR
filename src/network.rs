...
fn route_local_ip() -> Option<IpAddr> {
    ...
    for probe in &["10.20.1.1:80", "10.0.0.1:80", "10.20.0.1:80"] {
...
mod tests {
    ...
    use crate::{
        constants::{
            DEFAULT_ACCEPTED_STATUSES, DEFAULT_ACTIVE_TIMEOUT_MS,
            DEFAULT_CLEAN_OFFLINES_AFTER_DAYS, DEFAULT_CONNECT_TIMEOUT_MS,
            ...
            DEFAULT_PRIORITIZE_STABILITY, DEFAULT_PROBE_CONCURRENCY,
            DEFAULT_PROBE_PROCESS_CONCURRENCY, DEFAULT_PROXY_HEALTH_CHECK_INTERVAL,
            DEFAULT_PROXY_HEALTH_CHECK_URL, DEFAULT_REFRESH_SECONDS, DEFAULT_RETURN_CONFIGS_ASAP,
            DEFAULT_ROTATING_PROXY, DEFAULT_SCAN_ALL_CONFIGS, DEFAULT_SING_BOX_PATH,
            DEFAULT_STARTUP_TIMEOUT_MS, DEFAULT_TEST_URL, DEFAULT_TOP_N, DEFAULT_USE_CACHE_ONLY,
        },
        model::RuntimeConfig,
    };
    ...
    fn runtime_config(bind: &str, sharing_enabled: bool) -> RuntimeConfig {
        RuntimeConfig {
            ...
            use_cache_only: DEFAULT_USE_CACHE_ONLY,
            emergency_config: None,
            clean_offlines_after_days: DEFAULT_CLEAN_OFFLINES_AFTER_DAYS,
            geoip_db_path: None,
            sharing_enabled,
            require_token: false,
            token: String::new(),
            probe_mode: "active".to_string(),
            sing_box_path: DEFAULT_SING_BOX_PATH.to_string(),
            connect_timeout_ms: DEFAULT_CONNECT_TIMEOUT_MS,
            speedtest_enabled: false,
            probe_concurrency: DEFAULT_PROBE_CONCURRENCY,
            probe_batch_size: None,
            probe_process_concurrency: DEFAULT_PROBE_PROCESS_CONCURRENCY,
            ...
            health_check_url: DEFAULT_PROXY_HEALTH_CHECK_URL.to_string(),
    ...
    fn enabled_loopback_bind_can_display_lan_sharing_url() {
        ...
        let hosts = vec!["10.20.43.1".to_string()];
    ...
    fn keeps_only_primary_detected_host() {
        ...
        let hosts = discoverable_hosts_from_ips(vec![
            "10.20.1.87".parse::<IpAddr>().expect("valid IP"),
            "10.5.0.2".parse::<IpAddr>().expect("valid IP"),
            "10.20.197.1".parse::<IpAddr>().expect("valid IP"),
        ]);
...
Wireless LAN adapter Wi-Fi:
    IPv4 Address. . . . . . . . . . . : 10.20.43.1(Preferred)
    Subnet Mask . . . . . . . . . . . : 255.255.255.0
",
        );

        assert!(ips.contains(&"10.20.43.1".parse::<IpAddr>().expect("valid IP")));
    }
    ...
    fn filters_virtual_windows_adapters() {
        let ips = parse_ipconfig_ips(
            r"
Wireless LAN adapter Wi-Fi:
    IPv4 Address. . . . . . . . . . . : 10.20.1.87(Preferred)
    Subnet Mask . . . . . . . . . . . : 255.255.255.0
...
Tunnel adapter Teredo Tunneling Pseudo-Interface:
    IPv4 Address. . . . . . . . . . . : 10.20.1.100

Ethernet adapter VMware Network Adapter VMnet1:
    IPv4 Address. . . . . . . . . . . : 10.20.197.1
...
",
        ...
        assert!(ips.contains(&"10.20.1.87".parse::<IpAddr>().expect("valid IP")));
        assert!(!ips.contains(&"10.20.1.100".parse::<IpAddr>().expect("valid IP")));
        assert!(!ips.contains(&"10.20.197.1".parse::<IpAddr>().expect("valid IP")));
...
",
    ...
    fn filters_virtual_linux_interfaces() {
        let ips = parse_ip_addr_ips(
            "1: lo    inet 127.0.0.1/8 scope host lo\n\
             ...
             3: wlan0    inet 10.20.1.87/24 brd 10.20.1.255 scope global wlan0\n\
    ...
    fn is_private_lan_subnet_covers_common_ranges() {
        ...
        assert!(is_private_lan_subnet("10.20.1.1".parse().unwrap()));
    ...
    fn lan_conflicts_detected_in_sharing_status() {
        let config = runtime_config("10.20.1.87:27141", true);
        let hosts = vec!["10.20.1.87".to_string()];
