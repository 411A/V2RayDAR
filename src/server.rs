...
mod tests {
    ...
    fn blocks_lan_request_when_lan_sharing_is_disabled() {
        let config = runtime_config(false, false);
        let error = authorize_request(&config, addr("10.20.1.50:50000"), None)
    ...
    fn allows_lan_request_when_open_sharing_is_enabled() {
        ...
        assert!(authorize_request(&config, addr("10.20.1.50:50000"), None).is_ok());
    }
    ...
    fn requires_token_for_lan_when_enabled() {
        ...
        assert!(authorize_request(&config, addr("10.20.1.50:50000"), Some("wrong")).is_err());
        assert!(authorize_request(&config, addr("10.20.1.50:50000"), Some("secret")).is_ok());
    }
