...
pub fn draw(frame: &mut Frame<'_>, area: Rect, state: &TuiState) {
    ...
    let guide_text = if state.view == MenuView::Logs {
        ...
        "Tab change focus | Up/Down or j/k nav | Enter toggle/add | e edit | Esc/Ctrl+H back | :save :q"
    } else if state.selected_found.is_some() {
        "Enter set proxy | Esc cancel selection"
    } else {
        "Tab change focus | Up/Down or j/k nav | Enter select/edit | Esc/Ctrl+H back | :save :q"
    };
